...
[data-theme="dark"] {
  ...
  --menu-border: var(--tertiary-color);

  --hover-bg: rgba(75, 101, 124, 0.8);
}
...
.button-ui {
  ...
  border: 0.1em solid var(--btn-border);
...
.li:hover {
  border-left: 0.4em solid var(--btn-border);
...
.sidebar {
  ...
  border-bottom: 0.1em solid var(--menu-border);
}
/* ---------------------- Assignment Status ---------------------- */
.assignment-status {
  margin: 0.5em 0;
...
.authentication-background-wrapper {
  position: relative;
  width: 100%;
  height: 100vh;
  background: linear-gradient(
    to top,
    rgba(33, 53, 71, 0.87) 40%,
    transparent 100%
  );
  display: flex;
  align-items: center;
...
.input-field-wrapper input:focus::placeholder,
.input-field-wrapper input:not(:placeholder-shown)::placeholder {
